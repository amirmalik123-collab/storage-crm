...
async function initDb() {
  ...
  // A: Add intro rate fields if they don't exist yet (safe to run repeatedly)
  await db.query(`ALTER TABLE containers ADD COLUMN IF NOT EXISTS intro_rate NUMERIC(10,2) DEFAULT NULL`);
  await db.query(`ALTER TABLE containers ADD COLUMN IF NOT EXISTS intro_months INTEGER DEFAULT NULL`);
  await db.query(`ALTER TABLE rental_history ADD COLUMN IF NOT EXISTS intro_rate NUMERIC(10,2) DEFAULT NULL`);
  await db.query(`ALTER TABLE rental_history ADD COLUMN IF NOT EXISTS intro_months INTEGER DEFAULT NULL`);
...
app.get('/api/dashboard', authRequired, async (req, res) => {
  try {
    ...
    // B: monthly_revenue uses only truly Occupied containers (not Reserved)
    // For occupied containers, use intro_rate if still within intro period, else monthly_rate
    const today2 = new Date(); today2.setHours(0,0,0,0);
    const effectiveRate = (c) => {
      if (c.intro_rate && c.intro_months && c.start_date) {
        const monthsIn = (today2.getFullYear() - new Date(c.start_date).getFullYear()) * 12
          + (today2.getMonth() - new Date(c.start_date).getMonth());
        if (monthsIn < c.intro_months) return parseFloat(c.intro_rate) || 0;
      }
    ...
    res.json({
      ...
      monthly_revenue: occupied.reduce((s,c) => s + effectiveRate(c), 0),
  ...
  try {
    ...
    const { rows } = await db.query(
      ...
      [d.container_number, d.container_size||'20ft', d.location||'Back Row',
       ...
       parseFloat(d.monthly_rate)||0, orNull(d.intro_rate)?parseFloat(d.intro_rate):null,
...
// ── Confirm move-in (Reserved → Occupied) ────────────────────────────────────
app.post('/api/containers/:id/confirm-movein', authRequired, async (req, res) => {
  try {
    ...
    // Set start_date to today if it is in the future (brings it into Occupied)
    const today = new Date().toISOString().split('T')[0];
    const { rows: updated } = await db.query(
      `UPDATE containers SET start_date=$1, updated_at=NOW() WHERE id=$2 RETURNING *`,
      [today, req.params.id]
    );
...
app.get('/api/dashboard/revenue-trend', authRequired, async (req, res) => {
  try {
    ...
    for (let i = months - 1; i >= 0; i--) {
      ...
      // Revenue = sum of monthly_rate for containers that were occupied during this month
      // Active containers: start_date <= last day of month AND (end_date IS NULL OR end_date >= first day of month)
      ...
      const { rows: h } = await db.query(`
        ...
        WHERE start_date <= $1
          ...
          AND start_date IS NOT NULL
      `, [last, first]);
      rows.push({ label, year: y, month: m, revenue: parseFloat(r[0].revenue) + parseFloat(h[0].revenue) });
    }
    res.json(rows);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/dashboard/occupancy-trend', authRequired, async (req, res) => {
  try {
    ...
    for (let i = months - 1; i >= 0; i--) {
      ...
      const first = `${y}-${String(m).padStart(2,'0')}-01`;
      const last  = new Date(y, m, 0).toISOString().split('T')[0];
      // Occupied = containers + history where start<=last AND (end IS NULL OR end>=first)
      const { rows: c } = await db.query(`
        SELECT COUNT(*) AS n FROM containers
        WHERE start_date <= $1 AND (end_date IS NULL OR end_date >= $2) AND start_date IS NOT NULL
      `, [last, first]);
      const { rows: h } = await db.query(`
        SELECT COUNT(*) AS n FROM rental_history
        WHERE start_date <= $1 AND (end_date IS NULL OR end_date >= $2) AND start_date IS NOT NULL
      `, [last, first]);
      rows.push({ label, year: y, month: m, occupied: parseInt(c[0].n) + parseInt(h[0].n) });
    }
    res.json(rows);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/prospects', authRequired, async (req, res) => {
  try {
    ...
    const { rows } = await db.query(
      `SELECT * FROM prospects ${where} ORDER BY created_at DESC`, params
    );
    res.json(rows);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/prospects', authRequired, async (req, res) => {
  try {
    ...
    const { rows } = await db.query(
      `INSERT INTO prospects (name,email,phone,address1,address2,postcode,source,status,notes)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`,
      [name.trim(), email||'', phone||'', address1||'', address2||'', postcode||'',
...
app.put('/api/prospects/:id', authRequired, async (req, res) => {
  try {
    ...
    const { rows } = await db.query(
      ...
      [name.trim(), email||'', phone||'', address1||'', address2||'', postcode||'',
       source||'', status||'New', notes||'', req.params.id]
    );
...
app.get('/api/income', authRequired, async (req, res) => {
  try {
    ...
    const ps = fmtDate(periodStart);
    const pe = fmtDate(periodEnd);

    // All active containers occupied during this month
    const { rows: active } = await db.query(`
      SELECT id, container_number, container_size, location,
             customer_name, email, phone,
             monthly_rate, intro_rate, intro_months,
    ...
    // Archived rentals active during this month
    ...
    // Work out effective rate for the period month
    const effectiveRate = (c) => {
      if (c.intro_rate && c.intro_months && c.start_date) {
        const monthsIn =
          (y - new Date(c.start_date).getFullYear()) * 12 +
          (m - 1 - new Date(c.start_date).getMonth());
        if (monthsIn < parseInt(c.intro_months)) return parseFloat(c.intro_rate) || 0;
      }
      return parseFloat(c.monthly_rate) || 0;
    };

    // Calculate next billing date: same day-of-month as start_date, in or after period month
    const nextBillingDate = (c) => {
      if (!c.start_date) return null;
      const sd = new Date(c.start_date);
      const billingDay = sd.getDate();
      // Try billing day in the period month first
      const daysInPeriod = new Date(y, m, 0).getDate();
      const dayToUse = Math.min(billingDay, daysInPeriod);
      const candidate = new Date(y, m - 1, dayToUse);
      // If billing day hasn't occurred yet this month and we're in current month, that's next billing
      // Always return the billing date within the period month for display purposes
    ...
    const rows = [...active, ...hist].map(c => ({
      ...
      effective_rate:    effectiveRate(c),
      next_billing_date: nextBillingDate(c),
      billing_day:       c.start_date ? new Date(c.start_date).getDate() : null,
    }));

    // Sort by customer name then container number
    rows.sort((a, b) => a.customer_name.localeCompare(b.customer_name)
      || a.container_number.localeCompare(b.container_number));

    // Build daily income map for calendar: day -> { expected, count, tenants }
    ...
    const summary = {
      ...
      total_paid:     rows.filter(r => r.payment_status === 'Paid' || r.payment_status === 'Direct Debit')
                         .reduce((s, r) => s + r.effective_rate, 0),
      total_overdue:  rows.filter(r => r.payment_status === 'Overdue')
                         .reduce((s, r) => s + r.effective_rate, 0),
      total_pending:  rows.filter(r => r.payment_status === 'Pending')
                         .reduce((s, r) => s + r.effective_rate, 0),
      count: rows.length,
      daily,
    };

    res.json({ rows, summary, year: y, month: m });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
